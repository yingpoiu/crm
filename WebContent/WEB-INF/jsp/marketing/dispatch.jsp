...
<table class="query_form_table" cellSpacing=1 cellPadding=1>
	...
	<tr>
		<th>编号</th>
		<td>${sessionScope.chance.chcId }</td>
		<th>机会来源</th>
		<td>${sessionScope.chance.chcSource}</td>
	</tr>
	<tr>
		<th>客户名称</th>
		<td>${sessionScope.chance.chcCustName}</td>
		<th>成功机率（%）</th>
		<td>&nbsp;${sessionScope.chance.chcRate}</td>
	</tr>	
	<tr>
		<th>概要</th>
		<td colspan="3">${sessionScope.chance.chcTitle }</td>
	</tr>
	<tr>
		<th>联系人</th>
		<td>${sessionScope.chance.chcLinkman }</td>
		<th>联系人电话</th>
		<td>${sessionScope.chance.chcTel }</td>
	</tr>
	<tr>
		<th>机会描述</th>
		<td colspan="3">${sessionScope.chance.chcDesc}</td>
	</tr>
	<tr>
		<th>创建人</th>
		<td colspan="3">${sessionScope.user.usrName }</td>
	</tr>
</table>
<br />

<form action="sal/dispatchupdate?chc_id=${sessionScope.chance.chcId }"  method="post">
	...
	<tr>
		...
		<c:choose>
		  <c:when test="${sessionScope.user.usrRoleId<=3}">
			...
			<c:forEach items="${sessionScope.manager }" var="s">
				<option>${s.usrName }</option>
			</c:forEach>
